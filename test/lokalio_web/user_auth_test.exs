defmodule LokalioWeb.UserAuthTest do
  use LokalioWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Lokalio.Accounts
  alias Lokalio.Accounts.Scope
  alias LokalioWeb.UserAuth

  import Lokalio.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, LokalioWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      LokalioWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        LokalioWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
